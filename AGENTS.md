...
- **Deployment / cache notes:** `sw.js` caches the PWA shell under `dinner-by-destiny-v3`; increment that cache name whenever cached app-shell files change. `manifest.webmanifest` is configured for standalone mobile install.
