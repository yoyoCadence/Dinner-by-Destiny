...
- **Deployment / cache notes:** `sw.js` caches the PWA shell under `dinner-by-destiny-v14`; increment that cache name and `window.APP_VERSION` / `version.json` whenever cached app-shell files change. `manifest.webmanifest` is configured for standalone mobile install.
