...
- **Local server caveat:** Do not run `npm.cmd run start` or `node scripts/serve-static.mjs` as a foreground command in Codex, because the static server is long-running and makes the turn appear stuck. For browser checks, start it in the background with `Start-Process -WindowStyle Hidden -PassThru`, record the PID, and stop that PID after verification. If Browser / Playwright tooling is unavailable or returns no usable output, report the limitation and rely on `npm.cmd run test`.
- **Playwright on Windows caveat:** Do not invoke the Playwright skill's `.sh` wrapper directly from PowerShell; Windows may open a "choose an app for .sh files" dialog instead of running it. Prefer the in-app Browser plugin when available. If using the Playwright skill, run it through an explicit shell such as `bash.exe C:\Users\memor\.codex\skills\playwright\scripts\playwright_cli.sh --help` or `"C:\Program Files\Git\bin\bash.exe" C:\Users\memor\.codex\skills\playwright\scripts\playwright_cli.sh snapshot`. Do not fix this by changing the global Windows default app for `.sh` files, because Codex still may not receive stdout/stderr or arguments reliably. If `bash.exe` is unavailable or produces no usable output, report the limitation rather than retrying the `.sh` file.
- **GitHub CLI / sandbox network caveat:** If `gh auth status` works in the user's own PowerShell but fails inside Codex with `The token in GITHUB_TOKEN is invalid`, do not assume the token is wrong. First compare only a safe token fingerprint (length plus a short SHA-256 prefix) and inspect `HTTPS_PROXY`, `HTTP_PROXY`, and `ALL_PROXY`. Codex may set these proxy variables to `http://127.0.0.1:9`, which makes `gh` fail even with the same valid token. In that case, use the GitHub connector for PR/issue work, or request an escalated network command only when `gh` is truly required.
...
- **Deployment / cache notes:** `sw.js` caches the PWA shell under `dinner-by-destiny-v16`; increment that cache name and `window.APP_VERSION` / `version.json` whenever cached app-shell files change. `manifest.webmanifest` is configured for standalone mobile install.
